...
%% REWIRING FUNCTION FOR FORWARD TREES
function AllTrees = RewireNodes_Forward(AllTrees,AllParam,extendedNode,nearNodes)
...
%%            
            if dist > stepSize               
                [~,PathUtilityAlongNearNode,NodeUtility,fail] = ExtendMultiple_Fwd(nearest, target,nearestNodeID,AllTrees,AllParam,dist);    
            else              
                [PathUtilityAlongNearNode,NodeUtility,~,~] = UtilitySegment_Forward([extendedNode.x extendedNode.y],nearNodes{jj}.ID,AllTrees,AllParam,dist);           
                fail = false;
            end
%%
...
%%    
    for jj = 1:length(nearNodes)
        ...
        if (nearNodes{jj}.ID ~= pathTree(extendedNode.ID).parent)&&((abs(angDiff)/delta_t_max)<=psiDot_Constraint) 
            ...
            if dist > stepSize               
                [~,PathUtilityAlongExtendedNode,NodeUtility,fail] = ExtendMultiple_Fwd(nearest, target,nearestNodeID,AllTrees,AllParam,dist);       
            else             
                [PathUtilityAlongExtendedNode,NodeUtility,~,~] = UtilitySegment_Forward([nearNodes{jj}.x nearNodes{jj}.y],extendedNode.ID,AllTrees,AllParam,dist);               
                fail = false;
            end
%%               
