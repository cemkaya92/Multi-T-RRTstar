...
%% FUNCTION TO CALCULATE THE UTILITY OF EXTENDING A FORWARD TREE BRANCH
function [UtilityAlongPath,NodeUtility1,tr_test,AllTrees] = UtilitySegment_Forward(extended,PathNodeID,AllTrees,AllParam,dist)
...
x2 = AllTrees.ForwardTree(PathNodeID).x;  y2 = AllTrees.ForwardTree(PathNodeID).y;
length_segment = dist;

NodeUtility0 = AllTrees.ForwardTree(PathNodeID).NodeUtility;
...
%%
t0 = AllTrees.ForwardTree(PathNodeID).time;
t1 = AllTrees.ForwardTree(PathNodeID).time + (length_segment/AllTrees.ForwardTree(PathNodeID).vel);
delta_t = (t1-t0);
%% Calculation of REL is for forward tree
% Look at the Cumulative Risk Calculation for Forward Tree
%CumRisk(i+1) = 0.5*(exp(-lambda*t0)*R(i)+exp(-lambda*t1)*R(i+1))*delta_t + CumRisk(i);
REL = lambda*0.5*(exp(-lambda*t0)*NodeUtility0 + exp(-lambda*t1)*NodeUtility1)*delta_t; % this risk is a negative utility

UtilityAlongPath = REL + AllTrees.ForwardTree(PathNodeID).PathUtility;

%% Apply Transition Test (Rejection Sampling)
[tr_test,AllTrees.T_Forward] = transitionTest(NodeUtility0, NodeUtility1, AllTrees.T_Forward); % I can add the negative sign so that I can use Transition test as normal if it is defined in cost space not in utility space.
  
%% Update Transition Parameters
% update node min max temperatures
if NodeUtility1 < AllTrees.T_Forward.min
    AllTrees.T_Forward.min = NodeUtility1;
    AllTrees.T_Forward.range = AllTrees.T_Forward.max - AllTrees.T_Forward.min;
elseif NodeUtility1 > AllTrees.T_Forward.max
    AllTrees.T_Forward.max = NodeUtility1;
    AllTrees.T_Forward.range = AllTrees.T_Forward.max - AllTrees.T_Forward.min;
end
